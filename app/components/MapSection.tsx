...
export default function MapSection() {
  ...
  const { map } = getSiteContent(language);
  ...
  return (
    <section className="gold-ambient mx-auto w-[92%] overflow-hidden border border-t-0 border-[#2b241d] bg-[#0f0d0b] text-white sm:w-[90%]">
      ...
      <div className="fade-up delay-1 relative h-[320px] w-full overflow-hidden sm:h-[380px] lg:h-[440px]">
        ...
        <div className="absolute right-4 bottom-4 left-4 hidden border border-white/10 bg-[#11100e]/78 p-4 text-white backdrop-blur-sm sm:right-6 sm:bottom-6 sm:left-auto sm:max-w-[320px] sm:p-5 lg:block">
          ...
          <p className="mt-3 text-sm text-[#d4c4b2]/72">{map.cardSubtext}</p>
        </div>
      </div>
    </section>
  );
